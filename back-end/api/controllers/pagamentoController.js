...
exports.criarPagamentoPIX = async (req, res) => {
  try {
    ...
    if (!valor || Number(valor) <= 0) return res.status(400).json({ error: 'Valor do pagamento é obrigatório e deve ser maior que zero' });
    if (!descricao) return res.status(400).json({ error: 'Descrição do pagamento é obrigatória' });
...
exports.consultarPagamento = async (req, res) => {
  try {
    ...
    // atualiza DB se tiver id interno
    if (payment_id && !mercado_pago_id) {
      try {
    ...
    return res.status(200).json({
      ...
      pagamento: {
        ...
        valor: resultado.data.transaction_amount, // em reais
